...
qPCR <- read.delim(values, skip = 4, blank.lines.skip = TRUE, na.strings = "Undetermined")
endpos <- grep("Summary", qPCR$Well) - 1
qPCR <- read.delim(values, skip = 4, blank.lines.skip = TRUE, na.strings = "Undetermined", nrows = endpos)
qPCR <- tbl_df(qPCR)
...
# Save as qPCR
qPCR %>% 
  left_join(qPCR_legend) %>% 
  mutate(Sample = value) -> qPCR
