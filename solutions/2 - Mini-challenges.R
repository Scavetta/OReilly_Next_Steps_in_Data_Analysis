...
# remove the header:
qPCR <- read.delim("data/Atrogin1_values.txt", skip = 4)

# find the end of the data:
end.pos <- grep("\\s", qPCR$Well)[1] - 2

# read only until the end.pos:
qPCR <- read.delim("data/Atrogin1.txt", skip = 4, nrows = end.pos)

# ---- Mini-challenge II: Dealing with type mismatches ----
glimpse(qPCR)

# So we need to identify the problems here:
qPCR <- read.delim("data/Atrogin1.txt", skip = 4, stringsAsFactors = F,
                   nrows = end.pos, na.strings = "Undetermined")
